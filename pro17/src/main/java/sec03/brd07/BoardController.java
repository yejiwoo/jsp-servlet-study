		...
		try {
			...
			if((action == null) || 
			   ...
			   (action.equals("/listArticles.do"))) {
				...
				pw.print("<script>" 
				         +" alert('새글을 추가했습니다.');" 
						 +" location.href='"+request.getContextPath()+"/board170307/listArticles.do';"
				         +"</script>");
			 ...
			 //수정된 사용자의 글을 데이터베이스에 저장시킵니다. 
				...
				pw.print("<script>" 
				         ...
				         +"/board170307/viewArticle.do?articleNO="+ articleNO + "';"
				         +"</script>");
				return;
				...
				String strArticleNO = request.getParameter("articleNO");
			  ...
			  //답글을 작성할 수 있는 답글쓰기 창이 호출됩니다. 
				...
				System.out.println(strParentNO);
				
				//strParentNO = strParentNO.replace("\n", "");
				
				//부모글 번호를 가져와 저장합니다.
				//int parentNO = Integer.parseInt((String) strParentNO);
				...
				//session.setAttribute("parentNO", strParentNO);
			  ...
			  //작성된 답글을 데이터베이스에 저장합하고 글목록 페이지가 호출됩니다. 
				...
				String title = articleMap.get("title");
				String content = articleMap.get("content");
				//파일이름에 특수문자가 있을 경우 인코딩하여 저장합니다.
				String imageFileName = URLEncoder.encode(articleMap.get("imageFileName"), "UTF-8");
				
				//VO 객체에 작성된 답글정보를 저장합니다.
				articleVO.setParentNO(parentNO);
				articleVO.setId("lee");   //답글의 작성자 ID를 lee로 설정합니다.
				articleVO.setTitle(title);
				articleVO.setContent(content);
				articleVO.setImageFileName(imageFileName);
