	...
	private void doHandle(HttpServletRequest request, 
		...
		try {
			...
			if ((action == null) || 
				...
				int totalRowNum = boardDAO.selectTotArticles();

				paging.setTotalCount(totalRowNum);

				List<ArticleVO> articlesList = boardDAO.selectAllArticles(paging);
				...
				nextPage = "/boardpaging/listArticles.jsp";
...
/*			//전체 게시판 글을 표시합니다.
			if ((action == null) || 
				...
				nextPage = "/boardpaging/articleForm.jsp";
				...
				pw.print("<script>" 
				         +" alert('새글을 추가했습니다.');" 
						 +" location.href='"+request.getContextPath()+"/boardpaging/listArticles.do';"
				         +"</script>");
				...
				//호출(포워딩)페이지 이름 저장
				nextPage = "/boardpaging/viewArticle.jsp";
				...
				pw.print("<script>" 
				         ...
				         +"/boardpaging/viewArticle.do?articleNO="+ articleNO + "';"
				         +"</script>");
				return;
	          ...
	          //session 객체로부터 parentNO를 가져와, 답글의 정보로 테이블에 추가한다는 점입니다.
		        ...
		        //답글입력창이 표시됩니다.
				nextPage = "/boardpaging/replyForm.jsp";
