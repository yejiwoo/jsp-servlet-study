...
<body>
					...
					<td align='left'  width="35%">
						...
						<c:choose>
							<c:when test="${article.level > 1 }">
								...
								<a class="cls1" href="${contextPath}/board1703032/viewArticle.do?articleNO=${article.articleNO}">${article.title}</a>
							</c:when>
							<c:otherwise>
								<%-- <a class="cls1" href="${contextPath}/board170302/viewArticle.do?articleNO=${article.articleNO}">${article.title}</a> --%>
								<a class="cls1" href="${contextPath}/board1703032/viewArticle.do?articleNO=${article.articleNO}">${article.title}</a>
	...
	<div style="text-align:center; font-size:30px;">
		<p>board1703032 서브릿(시퀀스이용)</p>
		<a style="text-decoration:none;" 
		   href="${contextPath}/board1703032/articleForm.do">글쓰기</a></div>
</body>
</html>